...
def transcribe_source(source: str, language: str | None = None, keep_temp: bool = False) -> dict:
    ensure_api_key()
    work_dir = Path(tempfile.mkdtemp(prefix="chaunybili-transcribe-"))
    ...
    try:
        ...
        if not is_url(source):
            ...
            if is_video_path(path):
                ...
                extracted_audio = normalize_local_audio(path, work_dir)
                cleanup_paths.append(extracted_audio)
                local_result = transcribe_local_audio(extracted_audio, language=language)
                return {"success": True, "provider": "dashscope", "api_mode": "recognition", "model": "paraformer-realtime-v2", "source_kind": source_kind, "source": normalized_source, "submitted_source": str(extracted_audio), "result": local_result}
            if is_audio_path(path):
                source_kind = "local_audio"
                normalized_audio = normalize_local_audio(path, work_dir)
                cleanup_paths.append(normalized_audio)
                local_result = transcribe_local_audio(normalized_audio, language=language)
                return {"success": True, "provider": "dashscope", "api_mode": "recognition", "model": "paraformer-realtime-v2", "source_kind": source_kind, "source": normalized_source, "submitted_source": str(normalized_audio), "result": local_result}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Transcribe Bilibili media with DashScope.")
